...
기존 Servlet/JSP 프로젝트를 Spring Boot + Spring MVC 구조로 옮긴 형태이며, 현재 코드는 JSP 화면 기반 MVC 기능과 일부 `/api/v1` REST API를 함께 제공한다.
...
| REST API | 인증, 사용자, 식단, 음식 검색, 헬스 체크 API |
...
화면은 `/WEB-INF/views/**/*.jsp`에 있고, 컨트롤러는 문자열 view name을 반환한다. REST API는 `@RestController` 기반으로 JSON 응답을 반환한다.
...
MySQL 연결은 Spring `DataSource` 대신 `DBUtil`의 `DriverManager.getConnection(...)`을 직접 사용한다. DB 접속 정보는 `application.properties`와 `DBUtil`에 모두 존재하지만, 리포지토리 구현은 `DBUtil`의 하드코딩된 연결 정보를 사용한다.
...
REST API도 세션 기반 인증을 사용한다. 인증이 필요한 API에서 로그인 세션이 없으면 `LoginCheckFilter`가 `401` JSON을 반환한다.

REST 예외는 `ApiExceptionHandler`가 `ProblemDetail` 형식으로 처리한다. JSP 화면 예외는 `GlobalExceptionHandler`가 `error/error.jsp`를 반환한다.

Swagger UI는 `/swagger-ui/index.html`에서 확인할 수 있도록 설정되어 있다.
...
기술적으로는 Servlet/JSP 프로젝트를 Spring MVC 계층 구조로 정리하고, 일부 REST API와 Swagger 문서를 추가한 상태다. 다만 전체가 JPA나 MyBatis로 통합된 구조는 아니며, 직접 JDBC와 일부 메모리 저장소가 혼재한다.

## 14. 구현상 주의할 점

현재 구현을 바탕으로 추가 개발할 때 주의할 점은 다음과 같다.

- `DBUtil`에 DB 접속 정보가 하드코딩되어 있어 `application.properties`와 불일치할 수 있다.
- 챌린지와 소셜 데이터는 메모리 저장소이므로 재시작 후 변경 사항이 사라진다.
- Spring Security의 인증/인가 기능은 적극적으로 사용하지 않고, 자체 필터가 접근 제어를 담당한다.
- REST API는 인증/사용자/식단/음식/헬스 체크까지만 제공되며, 커뮤니티/챌린지/소셜 REST API는 아직 없다.
...
현재 완성도가 높은 영역은 세션 인증, 사용자/식단/음식 검색, 영양 계산, 식단 분석, 커뮤니티 CRUD, JSP 화면 흐름, 일부 REST API다. 반면 챌린지와 소셜은 기능 화면은 갖추었지만 저장소가 메모리 기반이라 운영 영속성은 부족하다.

따라서 현재 프로젝트는 “Spring MVC/JSP 기반 건강 코칭 서비스의 기본 기능 구현체”이며, 다음 단계에서는 배치 기반 데이터 적재, 챌린지/소셜 DB 영속화, REST API 확장, 보안 설정 정교화, 실제 AI 또는 규칙 엔진 분리 등을 붙이기 좋은 상태다.
