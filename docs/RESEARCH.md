...
| REST API | 인증, 사용자, 식단, 음식 검색, 헬스 체크, 영양 DB 배치 실행 API |
...
- REST API는 인증/사용자/식단/음식/헬스 체크/영양 배치 실행까지만 제공되며, 커뮤니티/챌린지/소셜 REST API는 아직 없다.
...
현재 완성도가 높은 영역은 세션 인증, 사용자/식단/음식 검색, 영양 계산, 식단 분석, 커뮤니티 CRUD, JSP 화면 흐름, 일부 REST API, 영양성분 DB 배치 적재다. 반면 챌린지와 소셜은 기능 화면은 갖추었지만 저장소가 메모리 기반이라 운영 영속성은 부족하다.

따라서 현재 프로젝트는 “Spring MVC/JSP 기반 건강 코칭 서비스의 기본 기능 구현체”이며, 다음 단계에서는 챌린지/소셜 DB 영속화, REST API 확장, 보안 설정 정교화, 배치 운영 권한 관리, 실제 AI 또는 규칙 엔진 분리 등을 붙이기 좋은 상태다.
