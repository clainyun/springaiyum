...
기존 Servlet/JSP 프로젝트를 Spring Boot + Spring MVC 구조로 옮긴 뒤, 같은 기능을 `client` 디렉터리의 Vue 3 + Vite SPA에서 사용할 수 있도록 `/api/v1` REST API를 확장한 상태다. 일반 사용자 화면 경로는 Vue SPA가 담당하고, 기존 JSP 화면 기반 MVC 기능은 `/legacy/**` 경로에 남아 있다.
...
기존 JSP 화면은 `/WEB-INF/views/**/*.jsp`에 있고, 레거시 컨트롤러는 `/legacy/**`에서 문자열 view name을 반환한다. Vue SPA 화면은 `client/src/views/**/*.vue`에 있으며, `client/src/composables`의 Axios 기반 API composable로 Spring REST API를 호출한다. `pnpm build` 산출물은 `src/main/resources/static`에 생성되어 Spring Boot가 `index.html`과 정적 asset을 서빙할 수 있다.
...
- Vue SPA는 Vite 개발 서버에서 실행할 수 있고, `/api`, `/batch`, `/swagger-ui`, `/v3/api-docs` 요청을 Spring Boot 서버로 proxy한다. 운영 빌드 산출물은 `src/main/resources/static`에 둔다.
...
현재 완성도가 높은 영역은 세션 인증, 사용자/식단/음식 검색, 영양 계산, 식단 분석, 커뮤니티 CRUD, Vue SPA 화면 흐름, `/legacy/**` JSP 화면 흐름, REST API, 영양성분 DB 배치 적재다. 반면 챌린지와 소셜은 기능 화면과 API는 갖추었지만 저장소가 메모리 기반이라 운영 영속성은 부족하다.

따라서 현재 프로젝트는 “Spring MVC/JSP 기반 건강 코칭 서비스를 Vue SPA 중심으로 전환한 구현체”이며, 다음 단계에서는 챌린지/소셜 DB 영속화, 레거시 JSP 파일 제거, 보안 설정 정교화, 배치 운영 권한 관리, 실제 AI 또는 규칙 엔진 분리 등을 붙이기 좋은 상태다.
